 ...
 * 2014-12-03 rwezowicz      Added Byte[] in/out to the CopyAndConvert
 */
...
namespace Common.Audio
{
	...
	public class WAVFile
	{
		...
		/// <summary>
		/// Opens a WAV file and attempts to read the header & audio information.
		/// </summary>
		...
		/// <summary>
		/// Opens a WAV file buffer and attempts to read the header & audio information.
		/// </summary>
		/// <param name="pFilename">The name of the file to open</param>
		/// <returns>A blank string on success, or a message on failure.</returns>
		public String Open(byte[] myfilebtytes)
		{
			String retval = "";

			try
			{
				mFileStream = new MemoryStream(myfilebtytes);
				OpenStream(mFileStream);
			}
			catch (Exception exc)
			{
		...
		/// <summary>
		/// Attempts to read the header and audio information from the inbound stream
		/// </summary>
		/// <param name="bytesstream"></param>
		private void OpenStream(Stream bytesstream)
		{
			// RIFF chunk (12 bytes total)
			// Read the header (first 4 bytes)
			byte[] buffer = new byte[4];
			bytesstream.Read(buffer, 0, 4); // first four bytes are "RIFF"
			buffer.CopyTo(mWAVHeader, 0);
			// Read the file size (4 bytes)
			bytesstream.Read(buffer, 0, 4);
			...
			// Format chunk (24 bytes total; may contain additional 36 bytes of JUNK chunk)
			// "fmt " (ASCII characters)
			bytesstream.Read(buffer, 0, 4);
			// if the wav header contains the JUNK chunk, we need to skip over it
			var junkHeader = new byte[] {74, 85, 78, 75}; // JUNK chunk is 36 bytes total
			if (buffer.SequenceEqual(junkHeader))
			{
				bytesstream.Seek(36, SeekOrigin.Current);
			}

			// Length of format chunk (always 16)
			bytesstream.Read(buffer, 0, 4);
			// 2 bytes (value always 1)
			bytesstream.Read(buffer, 0, 2);
			// # of channels (2 bytes)
			bytesstream.Read(buffer, 2, 2);
			...
			// Total of 44 bytes read up to this point. (80 bytes if JUNK chunk present)
			...
			{
				if (mDataSizeBytes != bytesstream.Length - 36)
					mDataSizeBytes = (int)(bytesstream.Length - 36);
			}
		...
		private void DoCreate(Stream mystream, bool pStereo, int pSampleRate, short pBitsPerSample)
		{
			...
			try
			{
				...
				mystream.Write(buffer_2bytes, 0, 2);
				// Bits per sample (2 bytes)
		...
		public void CreateByteArray(bool pStereo, int pSampleRate, short pBitsPerSample)
		{
			InitMembers();

			mFileStream = new MemoryStream();

			DoCreate(mFileStream, pStereo, pSampleRate, pBitsPerSample);
		}
		...
		public static void CopyAndConvert(String pSrcFilename, String pDestFilename, short pBitsPerSample, bool pStereo, double pVolumeMultiplier)
		{
			...
			CreateWAVFile(srcFile, destFile, pBitsPerSample, pStereo, pVolumeMultiplier);
		...
		/// <summary>
		/// Converts a WAV Buffer's bits/sample and number of channels to a separate WAV file.
		/// </summary>
		/// <param name="bSrcFiledata">The byte array of the buffer to convert</param>
		/// <param name="pBitsPerSample">The destination's number of bits/sample</param>
		/// <param name="pStereo">Whether or not the destination should be stereo</param>
		public static byte[] CopyAndConvertToByteArray(byte[] bSrcFiledata, short pBitsPerSample, bool pStereo)
		{
			return CopyAndConvertToByteArray(bSrcFiledata, pBitsPerSample, pStereo, 1.0);
		}

		/// <summary>
		/// Converts a WAV file buffer's bits/sample and number of channels to a separate WAV file.
		/// </summary>
		/// <param name="bSrcFiledata">The WAV buffer to convert</param>
		/// <param name="pBitsPerSample">The destination's number of bits/sample</param>
		/// <param name="pStereo">Whether or not the destination should be stereo</param>
		/// <param name="pVolumeMultiplier">A multiplier that can be used to adjust the volume of the output audio file</param>
		public static byte[] CopyAndConvertToByteArray(byte[] bSrcFiledata, short pBitsPerSample, bool pStereo, double pVolumeMultiplier)
		{
			WAVFile srcFile = new WAVFile();
			String retval = srcFile.Open(bSrcFiledata);
			if (retval != "")
				throw new WAVFileException(retval, "WAVFile.Convert_Copy()");

			WAVFile destFile = new WAVFile();

			destFile.CreateByteArray(pStereo, srcFile.SampleRateHz, pBitsPerSample);

			CreateWAVFile(srcFile, destFile, pBitsPerSample, pStereo, pVolumeMultiplier);

			destFile.FinalizeStream();

			return ((MemoryStream)destFile.mFileStream).ToArray();
		}


		private static void CreateWAVFile(WAVFile srcFile, WAVFile destFile, short pBitsPerSample, bool pStereo, double pVolumeMultiplier)
		{
		...
		private Stream mFileStream; // For reading the audio file
